...
import { emptyCropGuide, slugify, type CropGuide } from '../../../data/resources';

type EditorTab = 'Basic Info' | 'Content' | 'SEO';
const TABS: EditorTab[] = ['Basic Info', 'Content', 'SEO'];

interface CropGuideEditorProps {
  guide: CropGuide | null;
  onSave: (guide: Omit<CropGuide, 'id'> & { id?: string }) => Promise<void>;
  onClose: () => void;
}
...
/** Tabbed editor for Crop Guides — mirrors ArticleEditor.tsx, with a crop field and optional PDF download in place of author/category. */
export function CropGuideEditor({ guide, onSave, onClose }: CropGuideEditorProps) {
  const [activeTab, setActiveTab] = useState<EditorTab>('Basic Info');
  const [form, setForm] = useState<Omit<CropGuide, 'id'> & { id?: string }>(guide ?? emptyCropGuide);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState('');

  const update = <K extends keyof CropGuide>(key: K, value: CropGuide[K]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Could not save crop guide. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white w-full max-w-3xl max-h-[92vh] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 shrink-0">
          <h2 className="font-sans text-2xl font-bold text-primary">{guide ? 'Edit Crop Guide' : 'Add Crop Guide'}</h2>
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select value={form.status} onChange={(e) => update('status', e.target.value as CropGuide['status'])} className={inputClass}>
            ...
            {activeTab === 'Content' && (
              <div className="space-y-6">
                <ImageUploadField label="Cover Image" value={form.coverImage} onChange={(url) => update('coverImage', url)} folder="resources/crop-guides" previewClassName="w-full h-40 object-cover rounded-xl border border-slate-200" />
                <ImageUploadField label="PDF Download (optional)" value={form.pdfUrl} onChange={(url) => update('pdfUrl', url)} folder="resources/crop-guides" accept="application/pdf" />
                <div>
          ...
          <div className="flex justify-end gap-3 px-8 py-5 border-t border-slate-100 shrink-0">
            ...
            <button type="submit" disabled={isSaving} className="px-6 py-3 bg-primary text-secondary-container rounded-xl font-sans font-bold text-sm hover:bg-primary-container transition-colors disabled:opacity-60">
              {isSaving ? 'Saving...' : 'Save Crop Guide'}
            </button>
          </div>
        </form>
      </div>
