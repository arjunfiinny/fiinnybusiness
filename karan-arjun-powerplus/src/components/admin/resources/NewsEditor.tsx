...
import { emptyNews, slugify, type News } from '../../../data/resources';
...
interface NewsEditorProps {
  news: News | null;
  onSave: (news: Omit<News, 'id'> & { id?: string }) => Promise<void>;
  onClose: () => void;
}
...
/** Tabbed editor for News items — mirrors ArticleEditor.tsx's structure without the category/author fields, since news items are simpler timestamped announcements of company activity rather than authored editorial pieces. */
export function NewsEditor({ news, onSave, onClose }: NewsEditorProps) {
  const [activeTab, setActiveTab] = useState<EditorTab>('Basic Info');
  const [form, setForm] = useState<Omit<News, 'id'> & { id?: string }>(news ?? emptyNews);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState('');

  const update = <K extends keyof News>(key: K, value: News[K]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Could not save news item. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white w-full max-w-3xl max-h-[92vh] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 shrink-0">
          <h2 className="font-sans text-2xl font-bold text-primary">{news ? 'Edit News' : 'Add News'}</h2>
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select value={form.status} onChange={(e) => update('status', e.target.value as News['status'])} className={inputClass}>
            ...
            {activeTab === 'Content' && (
              <div className="space-y-6">
                <ImageUploadField label="Cover Image" value={form.coverImage} onChange={(url) => update('coverImage', url)} folder="resources/news" previewClassName="w-full h-40 object-cover rounded-xl border border-slate-200" />
                <div>
