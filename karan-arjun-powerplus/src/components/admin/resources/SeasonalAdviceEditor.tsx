...
import { emptySeasonalAdvice, type SeasonalAdvice } from '../../../data/resources';

interface SeasonalAdviceEditorProps {
  advice: SeasonalAdvice | null;
  onSave: (advice: Omit<SeasonalAdvice, 'id'> & { id?: string }) => Promise<void>;
  onClose: () => void;
}
...
export function SeasonalAdviceEditor({ advice, onSave, onClose }: SeasonalAdviceEditorProps) {
  const [form, setForm] = useState<Omit<SeasonalAdvice, 'id'> & { id?: string }>(advice ?? emptySeasonalAdvice);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState('');

  const update = <K extends keyof SeasonalAdvice>(key: K, value: SeasonalAdvice[K]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.advice.trim()) { setError('Advice content is required.'); return; }
    setError('');
    setIsSaving(true);
    try {
      await onSave(form);
      ...
      setError('Could not save seasonal advice. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white w-full max-w-lg max-h-[92vh] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 shrink-0">
          <h2 className="font-sans text-2xl font-bold text-primary">{advice ? 'Edit Seasonal Advice' : 'Add Seasonal Advice'}</h2>
          ...
          <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={form.season} onChange={(e) => update('season', e.target.value)} className={inputClass} placeholder="e.g. Kharif" />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea rows={5} required value={form.advice} onChange={(e) => update('advice', e.target.value)} className={`${inputClass} resize-none`} />
            </div>
            <ImageUploadField label="Cover Image" value={form.coverImage} onChange={(url) => update('coverImage', url)} folder="resources/seasonal-advice" previewClassName="w-full h-32 object-cover rounded-xl border border-slate-200" />
