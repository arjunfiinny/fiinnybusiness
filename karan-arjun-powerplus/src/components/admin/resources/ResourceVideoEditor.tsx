...
import { emptyResourceVideo, slugify, youTubeThumbnailUrl, type ResourceVideo } from '../../../data/resources';

interface ResourceVideoEditorProps {
  video: ResourceVideo | null;
  onSave: (video: Omit<ResourceVideo, 'id'> & { id?: string }) => Promise<void>;
  onClose: () => void;
}
...
/** Single-panel editor for a Resources video — mirrors components/admin/farmerSuccess/VideoEditor.tsx exactly, targeting the resourceVideos collection instead of farmerVideos (educational/informational videos, not testimonial-style). */
export function ResourceVideoEditor({ video, onSave, onClose }: ResourceVideoEditorProps) {
  const [form, setForm] = useState<Omit<ResourceVideo, 'id'> & { id?: string }>(video ?? emptyResourceVideo);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState('');

  const update = <K extends keyof ResourceVideo>(key: K, value: ResourceVideo[K]) => {
      ...
      <div className="bg-white w-full max-w-lg max-h-[92vh] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden">
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
            ...
            {thumbnail && <img src={thumbnail} alt="" className="w-full aspect-video object-cover rounded-xl border border-slate-200" />}
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>Type</label>
                <select value={form.kind} onChange={(e) => update('kind', e.target.value as ResourceVideo['kind'])} className={inputClass}>
